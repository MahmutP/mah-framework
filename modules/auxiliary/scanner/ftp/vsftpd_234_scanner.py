...
class vsftpd_234_scanner(BaseModule):
    ...
    def run(self, options: dict[str, Any]) -> str:
        ...
        print("\n" + "=" * 70)
        print("[*] [bold cyan]vsftpd 2.3.4 Backdoor Scanner[/bold cyan]")
        ...
        for host in hosts:
            ...
            if result["status"] == "VULNERABLE":
                print("[bold green]✓ VULNERABLE[/bold green]")
                ...
                print("[bold yellow]⚠ PATCHED[/bold yellow]")
                print(f"    └─ Banner: {result['banner']}")
                print(f"    └─ {result['details']}")
                stats["patched"] += 1
                ...
                print("[bold blue]✗ NOT VULNERABLE[/bold blue]")
                if result["banner"]:
                    print(f"    └─ Banner: {result['banner']}")
