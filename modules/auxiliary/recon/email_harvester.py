...
class email_harvester(BaseModule):
    ...
    def __init__(self):
        ...
        self.Options = {
            "DOMAIN": Option(
                "DOMAIN", "", True, "Sorgulanacak hedef domain (örn: example.com)"
            ),
    ...
    def run(self, options: dict[str, Any]):
        ...
        try:
            ...
            if not emails:
                print(
                    f"[yellow][!] '{url}' üzerinde e-posta adresi bulunamadı.[/yellow]"
                )
                return True

            table = Table(
                title=f"Extracted Emails for {domain}",
                show_header=True,
            ...
            print("[bold red][-] Hedefe bağlanılamadı (Zaman aşımı).[/bold red]")
            return False
        except requests.exceptions.ConnectionError:
            print(
                "[bold red][-] Bağlantı hatası: Sunucu bulunamadı veya bağlantı koptu.[/bold red]"
            )
            return False
