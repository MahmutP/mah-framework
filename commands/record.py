...
class Record(Command):
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        if subcommand == "start":
            ...
            print(
                "Çalıştırdığınız komutlar kaydedilecek. Durdurmak için [bold cyan]record stop <dosya>[/bold cyan] kullanın."
            )
            return True
            ...
            if len(args) > 1:
                ...
                try:
                    ...
                    print(
                        f"[bold green]✔ Kayıt durduruldu ve '{filename}' dosyasına yazıldı.[/bold green]"
                    )
                ...
                print(
                    "[bold yellow]⚠ Kayıt durduruldu.[/bold yellow] (Dosya adı belirtilmedi)"
                )
                if commands:
                    print(
                        f"[bold cyan]Kaydedilen Komutlar ({len(commands)}):[/bold cyan]"
                    )
                    for cmd in commands:
                        print(f"  {cmd}")
                else:
            ...
            if shared_state.is_recording:
                print("[bold green]● Kayıt DEVAM EDİYOR.[/bold green]")
                print(
                    f"Şu ana kadar kaydedilen komut sayısı: {len(shared_state.recorded_commands)}"
                )
            else:
