...
class HttpHeaderAnalyzer(BaseModule):
    def __init__(self):
        ...
        self.Description = (
            "Hedef web adresinin HTTP başlıklarını (headers) çeker ve analiz eder."
        )
        self.Author = "MahmutP"
    ...
    def run(self, options):
        ...
        try:
            ...
            if response.status_code == 405:
                print(
                    "[yellow][!][/yellow] HEAD isteği reddedildi (405). GET deneniyor..."
                )
                response = requests.get(target_url, timeout=timeout, stream=True)
                response.close()  # İçeriği indirmeden kapat

            print(
                f"[bold green][+][/bold green] Yanıt alındı! Durum Kodu: [bold cyan]{response.status_code}[/bold cyan]"
            )

            # Başlıkları tablo olarak gösterelim
            table = Table(
                title=f"HTTP Headers ({target_url})",
                show_header=True,
            ...
            if "x-powered-by" in response.headers:
                print(
                    f"[red]! Teknoloji Bilgisi İfşası (X-Powered-By):[/red] {response.headers['x-powered-by']}"
                )
