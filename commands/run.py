...
class Run(Command):
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        if not selected_module:
            print(
                "Çalıştırılacak bir modül seçili değil. Lütfen önce 'use <modül_yolu>' komutunu kullanın."
            )
            return False
        if not selected_module.check_required_options():
            print(
                f"[{selected_module.Name}] Modül çalıştırılamaz: Zorunlu seçenekler ayarlanmamış."
            )
            return False
        print(f"[{selected_module.Name}] Modül çalıştırılıyor...")
        try:
            current_options = {
                name: opt.value for name, opt in selected_module.get_options().items()
            }
            ...
            print(
                f"[{selected_module.Name}] Modül çalıştırılırken beklenmedik bir hata oluştu: {e}"
            )
