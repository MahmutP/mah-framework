...
pytestmark = pytest.mark.skipif(
    not HAS_REQUESTS_MOCK, reason="requests_mock kütüphanesi yüklü değil"
)
...
def test_analyze_network(tracker):
    ...
    tracker.fetch_profile_info = MagicMock(
        side_effect=[
...
def test_compare_users(tracker):
    ...
    u1_followers = [
        {"username": "common", "link": ""},
        {"username": "u1_only", "link": ""},
    ]
    u1_following = []
...
def test_run_full_integration(tracker):
    ...
    tracker.analyze_relationships = MagicMock(
        return_value={
            "mutual": set(),
            "not_following_back": set(),
            "not_followed_back": set(),
        }
    )
