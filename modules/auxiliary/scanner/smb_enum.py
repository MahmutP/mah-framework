...
class smb_enum(BaseModule):
    ...
    Description = (
        "Hedef sistemdeki SMB paylaşımlarını bulur ve anonymous giriş denemesi yapar."
    )
    Author = "Mahmut P."
    ...
    def run(self, options: dict[str, Any]):
        ...
        print(
            f"[bold cyan][*] {rhost}:{rport} SMB Paylaşımları aranıyor...[/bold cyan]"
        )
        ...
        try:
            ...
            print(
                f"[bold red][-] {rhost}:{rport} - Bağlantı reddedildi veya zaman aşımına uğradı. Port kapalı olabilir.[/bold red]"
            )
            return False
        ...
        try:
            ...
            if not connected:
                print("[bold red][-] SMB servisine bağlanılamadı.[/bold red]")
                return False
            ...
            if "SessionError" in str(type(e)) or "SessionError" in str(e):
                print(
                    "[yellow][!] Cihazda SMB açık ancak Anonymous (Guest) erişime izin verilmiyor.[/yellow]"
                )
                return True
