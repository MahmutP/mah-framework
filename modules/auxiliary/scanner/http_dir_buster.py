...
class HttpDirBuster(BaseModule):
    def __init__(self):
        ...
        self.Description = (
            "Web sunucusunda gizli veya açık dizinleri/dosyaları tespit eder."
        )
    ...
    def run(self, options):
        ...
        if not os.path.exists(wordlist_path):
            print(
                f"[bold red][!][/bold red] Wordlist dosyası bulunamadı: {wordlist_path}"
            )
            return False
        ...
        print(
            f"[bold blue][*][/bold blue] Tarama başlıyor... ({len(paths)} yol, {threads} thread)"
        )

        found_count = 0
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            future_to_url = {
                executor.submit(self.check_url, target_url, path): path
                for path in paths
            }
            for future in concurrent.futures.as_completed(future_to_url):
                result = future.result()
                if result:
                    path, status, _full_url = result
                    ...
                    print(
                        f"[{status_color}][+] /{path:<20} (Status: {status})[/{status_color}]"
                    )
                    found_count += 1
