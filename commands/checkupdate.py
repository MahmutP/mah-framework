...
class CheckUpdateCommand(Command):
    ...
    Name = "checkupdate"
    Description = (
        "Güncelleme kontrolü yapar. (--apply ile günceller, --backup ile yedekler)"
    )
    Category = "system"
    ...
    def _checkupdate_completer(self, text: str, word_before_cursor: str) -> list[str]:
        ...
        if word_before_cursor:
            return sorted(
                [opt for opt in options if opt.startswith(word_before_cursor)]
            )
    ...
    def _create_backup(self) -> bool:
        ...
        try:
            ...
            print(
                f"[*] Yedek oluşturuluyor: [bold]{backup_dir.name}/{backup_name}.zip[/bold]..."
            )

            shutil.make_archive(str(backup_path), "zip", str(framework_dir))

            print("[bold green]✓[/bold green] Yedekleme başarıyla tamamlandı.")
    ...
    def _apply_update(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "pull"], cwd=framework_dir, capture_output=True, text=True
            )

            if result.returncode == 0:
                print("[bold green]✓[/bold green] Kod güncellendi.")

                print(
                    "[*] Bağımlılıklar güncelleniyor (pip install -r requirements.txt)..."
                )
                subprocess.run(
                ...
                print("[bold green]✓[/bold green] Güncelleme başarıyla uygulandı!")
                logger.info("Otomatik güncelleme (self-update) başarıyla tamamlandı.")
                return True
            else:
                print(
                    f"[bold red]✗[/bold red] Güncelleme indirilemedi: {result.stderr}"
                )
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        parser.add_argument(
            ...
            "--apply",
            action="store_true",
            help="Güncellemeyi otomatik indir ve kur",
        )
        parser.add_argument(
            "-b", "--backup", action="store_true", help="Zorunlu yedek al"
        )
        ...
        try:
            ...
            print(
                "[bold red]✗[/bold red] Geçersiz argüman. (Kullanım: checkupdate [-a] [-b])"
            )
            return False
        ...
        print(
            f"[*] Mevcut versiyon: [bold]{local_version}[/bold] ({local_commits} commits)"
        )
        ...
        print(
            f"[*] Uzak versiyon:   [bold]{remote_version}[/bold] ({remote_commits} commits)"
        )
        ...
        if remote_commits > local_commits:
            ...
            print("[bold yellow]⚠ Güncelleme mevcut![/bold yellow]")
            ...
            if changelog:
                print(
                    "    [bold underline]Yeni Özellikler / Değişiklikler (Changelog):[/bold underline]"
                )
            ...
            if parsed_args.apply:
                ...
                print("    Güncellemek için:")
                print("    - [bold]checkupdate --apply[/bold] komutunu kullanın")
                print(
                    "    - Ya da manuel: [bold]git pull[/bold] ve [bold]pip3 install -r requirements.txt[/bold]"
                )
            ...
            print("[bold magenta]ℹ Lokal versiyon daha yeni![/bold magenta]")
            print(f"    {diff} commit push edilmedi.")
            print("    Push için: [bold]git push[/bold]")
        else:
            print("[bold green]✓ Güncel![/bold green]")
            print("    En son sürümü kullanıyorsunuz.")
