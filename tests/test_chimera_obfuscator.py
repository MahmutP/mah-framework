...
from build.chimera_obfuscator import (
    _ast_rename,
    _control_flow_flatten,
    _encrypt_strings,
    _inject_dead_code,
    _inject_junk,
    _inject_opaque_predicates,
    obfuscate,
)
...
def test_ast_rename():
    code, _mapping = _ast_rename(SAMPLE_CODE, seed=42)
    verify_syntax(code)
...
def test_inject_junk():
    code, _count = _inject_junk(SAMPLE_CODE, density=0.8)
    verify_syntax(code)
...
def test_inject_opaque_predicates():
    code, _count = _inject_opaque_predicates(SAMPLE_CODE, density=0.8)
    verify_syntax(code)
