...
class Info(Command):
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        if options:
            max_option_name_len = max(len(name) for name in options) if options else 0
            max_value_len = (
                max(len(str(opt.value)) for opt in options.values()) if options else 0
            )
            ...
            if shared_state.console_instance:
                with contextlib.suppress(AttributeError):
                    shared_state.console_instance.get_terminal_width()
            (
                4
                + min_option_name_width
                + 4
                + min_value_width
                + 4
                + min_required_width
            ...
            for name, opt in options.items():
                ...
                print(
                    f"    {name.ljust(min_option_name_width)}{' ' * 4}"
