...
class ResourceMonitor(BasePlugin):
    ...
    def get_hooks(self) -> dict[HookType, Callable[..., Any]]:
        return {HookType.PRE_COMMAND: self.on_pre_command}
    ...
    def _start_monitoring(self, silent: bool = False):
        ...
        if not silent:
            print(
                f"[Plugin] [{self.Name}] Kaynak izleme başlatıldı. Log: {self.log_file}"
            )
    ...
    def _monitor_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if time_diff < 0.1:
                    time_diff = 0.1  # Sıfıra bölünme hatasını önle
                ...
                try:
                    with open(self.log_file, "a", encoding="utf-8") as f:
                        f.write(f"[{datetime.now()}] ERROR: {e!s}\n")
                except:
                    pass
