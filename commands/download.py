...
class Download(Command):
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        if not shared_state.module_downloader:
            print(
                "[bold red]Hata:[/bold red] Modül İndirici (ModuleDownloader) yüklenemedi."
            )
            return False
        ...
        if subcommand == "search":
            ...
            if len(args) < 2:
                print(
                    "[bold red]Hata:[/bold red] Kullanım: download install <depo>/<modül_yolu>"
                )
                print(
                    "   Örnek: download install myrepo/auxiliary/scanner/my_scanner.py"
                )
                return False
            force = "--force" in args
            module_id = args[1]
            ...
            if len(args) < 2:
                print(
                    "[bold red]Hata:[/bold red] Kullanım: download verify <modül_yolu>"
                )
            ...
            print(
                "   Kullanılabilir alt komutlar: search, install, update, list, verify"
            )
            return False

    def _search_modules(self, search_term: str | None = None) -> None:
        ...
        for repo_name, repo_modules in by_repo.items():
            ...
            print(
                f"   {'Modül':<35} {'Versiyon':<10} {'Yazar':<15} {'İmza':<6} Açıklama"
            )
            print(
                f"   {'-----':<35} {'-------':<10} {'-----':<15} {'----':<6} --------"
            )
    ...
    def _list_installed(self) -> None:
        ...
        if not installed:
            ...
            print(
                "   Modül kurmak için: [bold]download install <depo>/<modül>[/bold]\n"
            )
            return
    ...
    def _check_updates(self) -> None:
        ...
        print(
            f"[bold yellow]⚠ {len(updates)} modül için güncelleme mevcut:[/bold yellow]\n"
        )
