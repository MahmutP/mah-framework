...
class Alias(Command):
    ...
    Category = "system"
    Aliases = []
    Usage = "alias <add|list|remove> [argümanlar]"
    Examples = [
        "alias list               # Tüm alias'ları listeler",
        "alias add h help         # 'h' yazınca 'help' çalışır",
    ...
    def _alias_completer(self, text: str, word_before_cursor: str) -> list[str]:
        ...
        if len(parts) == 1 and text.endswith(" "):
            ...
            len(parts) == 3 and parts[1].lower() == "remove" and not text.endswith(" ")
        ):
            ...
            if shared_state.command_manager:
                ...
                all_aliases = []
            return sorted(
                [alias for alias in all_aliases if alias.startswith(current_alias_part)]
            )
        return []
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        if subcommand == "add":
            ...
            print(
                f"Bilinmeyen 'alias' alt komutu: '{subcommand}'. Kullanım: alias <add|list|remove>"
            )
            return False
    ...
    def _list_aliases(self, aliases: dict[str, str]) -> bool:
        ...
        max_alias_len = max(len(alias_name) for alias_name in aliases) if aliases else 0
        max_target_len = (
            max(len(target_cmd) for target_cmd in aliases.values()) if aliases else 0
        )
        ...
        print(
            f"{' ' * LEFT_PADDING}{alias_header.ljust(max_alias_len)}{' ' * COL_SPACING}{target_command_header}"
        )
        print(
            f"{' ' * LEFT_PADDING}{'-' * max_alias_len}{' ' * COL_SPACING}{'-' * len(target_command_header)}"
        )
        for alias_name, target_cmd in sorted(aliases.items()):
            display_target_cmd = self._truncate_description(
                target_cmd, dynamic_max_target_len
            )
            print(
                f"{' ' * LEFT_PADDING}{alias_name.ljust(max_alias_len)}{' ' * COL_SPACING}{display_target_cmd}"
            )
        return True
