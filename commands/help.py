...
from core.cont import (  # dökümantasyon çıktılarının daha güzel olması için
    COL_SPACING,
...
class Help(Command):
    ...
    Name = "help"
    Description = (
        "Yardım menüsünü gösterir veya belirli bir komut hakkında bilgi verir."
    )
    Category = "core"
    Aliases = ["?"]
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        if args:
            ...
            resolved_command_name, _is_alias = command_manager.resolve_command(
                command_name
            )
            if not resolved_command_name:
                print(f"'{command_name}' adında bir komut veya alias bulunamadı.")
                return False
            cmd_obj: Command | None = command_manager.get_all_commands().get(
    ...
    def _display_general_help(self, command_manager: CommandManager) -> None:
        ...
        for category_display_name in sorted(categorized_commands.keys()):
            ...
            max_cmd_len = (
                max(len(cmd_name) for cmd_name in commands_in_category)
            ...
            print(
                f"{' ' * LEFT_PADDING}{cmd_header.ljust(max_cmd_len)}{' ' * COL_SPACING}{desc_header}"
            )
            print(
                f"{' ' * LEFT_PADDING}{'-' * max_cmd_len}{' ' * COL_SPACING}{'-' * len(desc_header)}"
            )
            for cmd_name, cmd_obj in sorted(commands_in_category.items()):
                ...
                print(
                    f"{' ' * LEFT_PADDING}{cmd_name.ljust(max_cmd_len)}{' ' * COL_SPACING}{display_description}"
                )
        print(
