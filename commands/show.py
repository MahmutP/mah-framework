...
from core.cont import COL_SPACING, DEFAULT_TERMINAL_WIDTH, LEFT_PADDING, TAB_SPING
...
class Show(Command):
    ...
    def _show_completer(self, text: str, word_before_cursor: str) -> list[str]:
        ...
        if len(parts) == 1 and text.endswith(" "):
            ...
            return sorted(
                [
                    cmd
                    for cmd in ["info", "modules", "options"]
                    if cmd.startswith(current_arg)
                ]
            )
        return []
    ...
    def execute(self, *args: str, **kwargs: Any) -> bool:
        ...
        module_manager: ModuleManager | None = shared_state.module_manager
        ...
        if subcommand == "modules":
            ...
            print(
                f"Bilinmeyen 'show' alt komutu: '{subcommand}'. Kullanım: show <modules|options|info>"
            )
            return False
    ...
    def _show_modules(self, module_manager: ModuleManager) -> bool:
        ...
        for category_display_name in sorted(categorized_modules.keys()):
            ...
            max_module_path_len = (
                max(len(path) for path in modules_in_category)
            ...
            print(
                f"{' ' * LEFT_PADDING}{module_header.ljust(max_module_path_len)}{' ' * (2 * TAB_SPING)}{desc_header}"
            )
            print(
                f"{' ' * LEFT_PADDING}{'-' * max_module_path_len}{' ' * (2 * TAB_SPING)}{'-' * len(desc_header)}"
            )
            for module_path, module_obj in sorted(modules_in_category.items()):
                description = module_obj.Description
                display_description = self._truncate_description(
                    description, dynamic_max_desc_len
                )
                print(
                    f"{' ' * LEFT_PADDING}{module_path.ljust(max_module_path_len)}{' ' * (2 * TAB_SPING)}{display_description}"
                )
        return True
    ...
    def _show_options(self) -> bool:
        ...
        if not options:
            print(
                f"Seçili modül '{selected_module.Name}' için ayarlanabilir seçenek bulunmamaktadır."
            )
            return True
        terminal_width = self._get_terminal_width()
        ...
        max_option_name_len = max(len(name) for name in options)
        ...
        for name, opt in options.items():
            ...
            display_description = self._truncate_description(
                opt.description, dynamic_max_desc_len
            )
            print(
                f"{' ' * LEFT_PADDING}{name.ljust(max_option_name_len)}{' ' * COL_SPACING}"
