...
class ssl_checker(BaseModule):
    ...
    Description = (
        "Belirtilen hedefteki SSL/TLS sertifikasının süresini ve durumunu kontrol eder."
    )
    Author = "Mahmut P."
    ...
    def run(self, options: dict[str, Any]):
        ...
        print(
            f"[bold cyan][*] {rhost}:{rport} TLS/SSL sertifikası kontrol ediliyor...[/bold cyan]"
        )
        ...
        try:
            ...
            try:
                ...
                ssl.get_server_certificate((rhost, rport))
                ...
                with socket.create_connection((rhost, rport), timeout=timeout) as sock2:
                    with context_dict.wrap_socket(
                        sock2, server_hostname=rhost
                    ) as ssock2:
            ...
            if cert_data:
                ...
                if "subject" in cert_data:
                    ...
                    print(
                        f"  [green]Issued To:[/green] {subject.get('commonName', 'Unknown')}"
                    )
                if "issuer" in cert_data:
                    issuer = dict(x[0] for x in cert_data["issuer"])
                    print(
                        f"  [green]Issuer:[/green] {issuer.get('organizationName', 'Unknown')}"
                    )
                ...
                if "notAfter" in cert_data:
                    ...
                    print(
                        f"  [green]Expires (UTC):[/green] {expire_date} ([{color}]{days_left} gün kaldı[/{color}])"
                    )
                ...
                print(
                    "[yellow][!] Cihazda SSL/TLS etkin ancak dict formatında veri okunamadı.[/yellow]"
                )
                return True
            ...
            print(
                f"[bold red][-] {rhost}:{rport} - Bağlantı zaman aşımına uğradı.[/bold red]"
            )
            return False
        except ConnectionRefusedError:
            print(
                f"[bold red][-] {rhost}:{rport} - Bağlantı reddedildi (Port kapalı olabilir).[/bold red]"
            )
            return False
        except Exception as e:
            print(f"[bold red][-] Beklenmeyen Hata: {e!s}[/bold red]")
            return False
